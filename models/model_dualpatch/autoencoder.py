...
from models.model_dualpatch.base.blocks import Encoder, Decoder , Decoder_unify
from models.model.quantizer.fsq import FSQ
from models import register


@register('autoencoder_dualpatch')
class AutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.decode(x_q)
        return_dict = {'pred_frames': x.contiguous()}  # 确保输出是连续的内存格式，避免潜在的性能问题
        return return_dict
    

# @register('autoencoder_first_token')
...
#             out_tokens=256,
#         )
#         self.quantize = FSQ(levels=[8, 8, 5, 5, 5])
#         self.decoder = Decoder(
#             model_size='small_thin',
#             patch_size=[4, 8, 8],
#             in_channels=token_size,
...
@register('autoencoder_first_token_res')
class AutoEncoder_first_token_res(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
